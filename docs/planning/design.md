...
> ⚠️ マークの付いた箇所は `decisions.md` の論点に対応する。**主要論点はユーザー確定済み**（A-3 PHP 7.4基準・8.1互換 / B-1 holiday_jp / C-3 コンストラクタ引数＋設定ローダー / D-1 CaoCsv は PHP 標準関数 / E-3 ドキュメント en/ja）。残る ⚠️ は実装時に詰める細部（例外クラス粒度・内部生成日付のタイムゾーン・apiclient の 8.1 実機検証）。
...
- `ext-mbstring`（caoCsv 用）を `require` に書くか `suggest`（`ext-mbstring`）にするかは実装時に確定（decisions.md H）。
...
go-heijitu の error 伝播を PHP の例外に置き換える。推奨案:

- ライブラリ共通の基底例外 `Heijitu\Exception\HeijituException`（インターフェース or 抽象基底）。
- 設定ファイル読み込み失敗・プロバイダーの取得失敗・認証情報不備などを、用途別の例外として throw。
- プロバイダー内部のエラーは握りつぶさず上位へ伝播（go-heijitu の方針を継承）。
- 具体的な例外クラスの粒度は要決定（`decisions.md`）。
...
- **タイムゾーンの明示**: PHP の `DateTimeImmutable` は生成時のタイムゾーンに依存する。日本の営業日計算では JST 固定が安全。ドキュメントで明記する。内部生成する日付（`firstBusinessDayOfMonth` 等）のタイムゾーン基準は実装時に確定（go 版は `time.Local`、decisions.md E-2）。
