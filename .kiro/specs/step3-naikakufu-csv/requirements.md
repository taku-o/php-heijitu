...
2. When `CaoCsv\Provider` is instantiated with a `csvPath` parameter pointing to a local file, the php-heijitu ライブラリ shall read and parse the CSV data from that local file path.
...
1. If the `mbstring` PHP extension is not loaded when `CaoCsv\Provider` is instantiated, the php-heijitu ライブラリ shall throw a `ProviderException` with a message that guides the user to install `ext-mbstring`.
